...
def generate_mock_response(message: str, system_prompt: str | None = None) -> str:
    ...
    if (
        "professional investment advisor" in lowered
        or "provide a recommendation for" in lowered
    ):
    ...
    if (
        "transaction was scored by our automated system with a borderline result"
        in lowered
    ):
        return (
